    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if domain in domain_to_filename:
                ...
                if domain == 'Imagery':  # Special handling for 'Imagery'
                    output, error = run_python_file(filename, user_prompt, timeout=25)

                    # Wait for 15 seconds for image generation
                    time.sleep(10)

                    # Try to open the image (assuming it's saved at image\out-0.png)
                    image_path = os.path.join('image', 'out-0.png')
                    if os.path.exists(image_path):
                        webbrowser.open(f"file://{os.path.realpath(image_path)}")
                    else:
                        output += "\nImage not found at the expected location."
                else:
                    output, error = run_python_file(filename, user_prompt, timeout=25)
